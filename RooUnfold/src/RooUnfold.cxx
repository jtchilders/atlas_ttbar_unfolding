...
void RooUnfold::GetErrMat()
{
  // Get covariance matrix from the variation of the results in toy MC tests
   cout << "[RooUnfold::GetErrMat] Ntoys = " << _NToys << "\n";
  if (_NToys<=1) return;
...
TH1* RooUnfold::Hreco (ErrorTreatment withError)
{
  ...
  for (Int_t i= 0; i < _nt; i++) {
     Int_t j= RooUnfoldResponse::GetBin (reco, i, _overflow); // returns i+1 if overflow = 0
...
RooUnfold* RooUnfold::RunToy(TH1 const *hData, TH1 const *hBkgd,const bool useGaus)
{
...
//      TVectorD newmeas(_meas->GetNbinsX());
//      TVectorD newerr (_meas->GetNbinsX());
//      for(unsigned int bin=1;bin<=_meas->GetNbinsX();++bin){
//         newmeas[bin-1] = _meas->GetBinContent(bin);
//         newerr [bin-1] = _meas->GetBinError(bin);
//      }
    const TVectorD& err= Emeasured();
    for (Int_t i= 0; i<_nm; i++) {
       if(hData && hBkgd)
          newmeas[i] = _rand.Poisson(hData->GetBinContent(i+1)) - _rand.Gaus(hBkgd->GetBinContent(i+1),hBkgd->GetBinError(i+1));
       else{
          if(useGaus){
             newmeas[i] = _rand.Gaus(newmeas[i],err[i]);
          }
          else
             newmeas[i] = _rand.Poisson(newmeas[i]);
       }
      //newmeas[i] = gRandom->Gaus(newmeas[i],err[i]);
    }
